...
void WorldMapGUI::drawMiniMap(pl::RenderTarget& window, const WorldMap& worldMap, pl::Vector2f playerPosition)
{
    ...
    miniMapRect.addQuad(pl::Rect<float>(-playerPositionMap.x * MINI_MAP_SCALE + MINI_MAP_WIDTH / 2, -playerPositionMap.y * MINI_MAP_SCALE + MINI_MAP_HEIGHT / 2,
        worldMap.getTexture().getWidth() * MINI_MAP_SCALE, worldMap.getTexture().getHeight() * MINI_MAP_SCALE),
        pl::Color(), pl::Rect<float>(0, 0, worldMap.getTexture().getWidth(), worldMap.getTexture().getHeight()));
    ...
    miniMapRect.addQuad(pl::Rect<float>(window.getWidth() - (MINI_MAP_WIDTH + MINI_MAP_PADDING) * intScale,
        ...
        pl::Rect<float>(0, 0, miniMapCutoutFramebuffer.getWidth(), miniMapCutoutFramebuffer.getHeight()));

    window.draw(miniMapRect, *Shaders::getShader(ShaderType::Default), &miniMapCutoutFramebuffer.getTexture(), pl::BlendMode::Alpha);
}
